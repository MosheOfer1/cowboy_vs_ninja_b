...
TEST_SUITE("Battle simulations")
{
    ...
    TEST_CASE("Run full battles using random_char to ensure full functionality")
    {
        ...
        SUBCASE("Team vs SmartTeam smart takes")
        {
            constexpr int NUM_SIMULATIONS = 1000;
            ...
            for (int sim = 0; sim < NUM_SIMULATIONS; sim++)
            {
                ...
                if (!team.stillAlive() && team2.stillAlive())
                {
                    smartTeamWins++;
                }
            }

            double winPercentage = static_cast<double>(smartTeamWins) / NUM_SIMULATIONS * 100.0;

            std::cout << "The SmartTeam won " << winPercentage << "% of the time." << std::endl;
            CHECK(winPercentage > 50);
        }
    }
    }